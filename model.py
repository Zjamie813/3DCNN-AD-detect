...
class Base3DModel(nn.Module):
    def __init__(self, num_seg_classes, f=8):
        ...
        self.conv.add_module('conv5',
                             nn.Conv3d(in_channels=4 * f, out_channels=32 * f, kernel_size=2, stride=1, padding=0,
                                       dilation=2))
        ...
        #                                dilation=2)) # bt,512,9,11,9
        # self.conv.add_module('conv14', nn.InstanceNorm3d(num_features=64 * f))
        # self.conv.add_module('conv15', nn.ReLU(inplace=True))
        # self.conv.add_module('conv16', nn.MaxPool3d(kernel_size=5, stride=2)) #64 * f * 3 * 4 * 3

        self.fc = nn.Sequential()
        self.fc.add_module('fc1', nn.Linear(64 * f * 9 * 11 * 9, 1024))
        #self.fc.add_module('relu1',nn.Sigmoid())
        self.fc.add_module('dp1', nn.Dropout(0.5))
        self.fc.add_module('fc2', nn.Linear(1024, num_seg_classes))
        self.fc.add_module('fc_relu1', nn.Softmax(dim=1))
        #self.fc_relu1 = nn.Softmax(dim=1)
        #self.fc.add_module('relu2', nn.Sigmoid())
